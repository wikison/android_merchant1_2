package com.zemult.merchant.view.password;
...
public class GridPasswordView extends LinearLayout implements PasswordView {
	...
	// private static final int DEFAULT_LINECOLOR = 0xF5F5F5;
	...
	private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		if (textSize != -1) {
			this.textSize = DensityUtil.px2sp(context, textSize);
		}

		lineWidth = (int) ta.getDimension(
				R.styleable.gridPasswordView_gridLineWidth,
	...
	private void inflaterViews(Context context) {
		...
		while (index < passwordLength) {
			...
			TextView textView = (TextView) inflater.inflate(
					R.layout.password_view_textview, null);
	...
	private TextWatcher textWatcher = new TextWatcher() {
		...
		public void onTextChanged(CharSequence s, int start, int before,
			...
			if (newStr.length() == 1) {
				...
				inputView.setText(passwordArr[1]);
				inputView.setSelection(1);
				inputView.addTextChangedListener(this);
			}
			else if (newStr.length() == 3) {
				String newNum = newStr.substring(2);
				for (int i = 0; i < passwordArr.length; i++) {
					if (passwordArr[i] == null) {
						passwordArr[i] = newNum;
						viewArr[i].setText(newNum);
				...
				inputView.removeTextChangedListener(this);
				inputView.setText(passwordArr[2]);
				inputView.setSelection(2);
				inputView.addTextChangedListener(this);
			}
			else if (newStr.length() == 4) {
				String newNum = newStr.substring(3);
				for (int i = 0; i < passwordArr.length; i++) {
					if (passwordArr[i] == null) {
						passwordArr[i] = newNum;
						viewArr[i].setText(newNum);
				...
				inputView.removeTextChangedListener(this);
				inputView.setText(passwordArr[3]);
				inputView.setSelection(3);
				inputView.addTextChangedListener(this);
			}
			else if (newStr.length() == 5) {
				String newNum = newStr.substring(4);
				for (int i = 0; i < passwordArr.length; i++) {
					if (passwordArr[i] == null) {
						passwordArr[i] = newNum;
						viewArr[i].setText(newNum);
				...
				inputView.removeTextChangedListener(this);
				inputView.setText(passwordArr[4]);
				inputView.setSelection(4);
				inputView.addTextChangedListener(this);
			}
			else if (newStr.length() == 6) {
				String newNum = newStr.substring(5);
				for (int i = 0; i < passwordArr.length; i++) {
					if (passwordArr[i] == null) {
						passwordArr[i] = newNum;
						viewArr[i].setText(newNum);
				...
				inputView.removeTextChangedListener(this);
				inputView.setText(passwordArr[5]);
				inputView.setSelection(5);
				inputView.addTextChangedListener(this);
			}
